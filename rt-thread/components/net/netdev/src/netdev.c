...
void netdev_low_level_set_link_status(struct netdev *netdev, rt_bool_t is_up)
{
    ...
    {   
        if (is_up)
        {
            ...
            {
                LOG_W("Not set internet up! ip addr is any, or netdev is not up.");
            }
    ...
    {
        LOG_W("Not change linkup state! netdev:%p; is linkup:%d", netdev, netdev == NULL ? 0 : netdev_is_link_up(netdev));
    }
}
