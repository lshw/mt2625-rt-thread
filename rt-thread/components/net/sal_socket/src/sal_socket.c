...
#define SAL_INTERNET_DATE_LEN  16
    ...
    if (!netdev)
    {
        LOG_E("In param error. F:%s; L:%d", __FUNCTION__, __LINE__);
        result = -RT_ERROR;
        return;
    }
    ...
    if (pf == RT_NULL || pf->skt_ops == RT_NULL || pf->netdb_ops == RT_NULL)
    {
        result = -RT_ERROR;
        LOG_E("In param error. F:%s; L:%d", __FUNCTION__, __LINE__);
        goto __exit;
...
__exit:
    if (result > 0)
    {
        LOG_I("Set network interface device(%s) internet status up.", netdev->name);
        netdev->flags |= NETDEV_FLAG_INTERNET_UP;

        /* execute link status change callback function */
        if (netdev->status_callback)
        {
            netdev->status_callback(netdev, NETDEV_CB_STATUS_INTERNET_UP);
        }
    }
    else
    {
        LOG_I("Set network interface device(%s) internet status down.", netdev->name);
        netdev->flags &= ~NETDEV_FLAG_INTERNET_UP;

        /* execute link status change callback function */
        if (netdev->status_callback)
        {
            netdev->status_callback(netdev, NETDEV_CB_STATUS_INTERNET_DOWN);
        }
    }
